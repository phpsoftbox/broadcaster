...
- `private.*` подходит для персональных/закрытых каналов, где нужна только проверка доступа.
- `presence.*` подходит для комнат с метаданными участника; обработчик обычно возвращает массив для `channel_data`.
- В текущей реализации Pushr `presence` не генерирует системные события join/leave автоматически.
